...
// 5 - Crie um endpoint  que utilize o método POST da entidade users que será responsável
// por cadastrar um usuário em um array de usuários.
/**
 * 7 - Adicione, uma validação no item 1 (Criar conta): o usuário deve possuir mais do que 18
...
/**
 * 6 -  Crie um método GET na entidade users função que será responsável por pegar todos
        ...
        if (item.cpf === cpf && item.name === name) {
          item.statement.map((item) => {
            if(item.date < actualDate){
              //terminar verificação de data no desafio 6 
              console.log('ontem',typeof item.date)
